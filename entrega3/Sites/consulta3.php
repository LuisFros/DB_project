...
<?php
  ...
  try {
    ...
    $usuario3 = $_POST["usuario_3"];

//tid   date    id_out  id_in   quantity
        $query= "SELECT transaccion.tid,MAX(transaccion.date) as Fecha,id_in,id_out,quantity 
                 FROM transaccion,usuarios
                 WHERE usuarios.username='$usuario3' AND (usuarios.id=transaccion.id_in OR  usuarios.id=transaccion.id_out) 
                 GROUP BY transaccion.tid;";
