...
<?php
  ...
  try {
    ...
    $usuario7 = $_POST["usuario_7"];
    $query = "SELECT (t1.p1-t2.p2) AS Cantidad
            FROM (SELECT COALESCE(SUM(quantity),0) AS p1
            FROM transaccion
            WHERE transaccion.id_in=(SELECT usuarios.id FROM usuarios WHERE usuarios.username='$usuario7')) AS t1
            ,(SELECT COALESCE(SUM(quantity),0) AS p2
            FROM transaccion
            WHERE transaccion.id_out=(SELECT usuarios.id FROM usuarios WHERE usuarios.username='$usuario7'))AS t2; ";
