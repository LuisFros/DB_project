      ...
      <div class="row" style="margin: 0px 5px;">
        <?php
          ...
          try {$db = new PDO("pgsql:dbname=".DATABASE.";host=".HOST.";port=".PORT.";user=".USER.";password=".PASSWORD);}
         	...
         	$query = "SELECT tipo, AVG(cambio) FROM PrecioHistorico WHERE (EXTRACT(MONTH FROM fecha) = EXTRACT(MONTH FROM CURRENT_TIMESTAMP) - 1) GROUP BY tipo;";
        	...
        	$rows = $result -> fetchAll();
              ...
              echo "<tr>";
                ...
                echo "<th>Tipo de cambio promedio</th>";
              echo "</tr>";
            	foreach ($rows as $row) {
                  if ($row[0] == "clp") {
                    $type = "Peso Chileno (CLP)";
                    $avg = "$" . number_format($row[1], 0, ",", ".");
                  } elseif ($row[0] == "usd") {
                    $type = "Dolar Americano (USD)";
                    $avg = "$" . number_format($row[1], 2, ",", ".");
                  }
                  echo "<tr>";
                    echo "<td>$type</td>";
                    echo "<td>$avg</td>";
